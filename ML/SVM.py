...
for i in range(1, 6):
    train_file = open('cifar-10-python/cifar-10-batches-py/data_batch_' + str(i), 'rb')
...
test_data = []
test_label = []
test_file = open('cifar-10-python/cifar-10-batches-py/test_batch', 'rb')
...
svm = SVM()
print("开始训练SVM...")
train_acc_history = svm.train(train_data, train_label, learning_rate=1e-7, reg=5e4)
print("训练完成！")
result= svm.predict(test_data)
#   使用训练好的模型对测试集数据进行预测并统计正确率
test_acc = np.mean(test_label == result)
print('在测试集上的准确率为: %f' % (np.mean(test_label == result)))
# 计算召回率和精确率
recall = recall_score(test_label, result, average='macro')
precision = precision_score(test_label, result, average='macro')

# 打印召回率和精确率
print("召回率（Macro）: %f" % recall)
print("精确率（Macro）: %f" % precision)
...
# 可视化混淆矩阵
conf_matrix = confusion_matrix(test_label, result)
